...
int main(int argc, char* argp[])
{
    ...
    // Bouw het pad naar game.nes op vanuit de map waar de EBOOT staat
    GetModulePath(RomPath, sizeof(RomPath));
    _strcat(RomPath, "roms/game.nes");

    // Laad de ROM rechtstreeks in plaats van de filebrowser te openen
    if (!NES_Init(RomPath)) {
        // Mocht game.nes niet gevonden worden, sluit de applicatie veilig af
        sceKernelExitGame();
        return 0;
    }
